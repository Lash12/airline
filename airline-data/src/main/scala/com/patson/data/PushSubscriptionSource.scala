...
object PushSubscriptionSource {
  ...
  private def endpointHash(endpoint: String): String =
    ...
    Using.resource(Meta.getConnection()) { connection =>
      ...
      Using.resource(connection.prepareStatement(
        s"""INSERT INTO $PUSH_SUBSCRIPTION_TABLE
           |(airline, endpoint, endpoint_hash, p256dh_key, auth_key, created_cycle, last_pushed_notification_id, failure_count, user_agent)
           |VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        statement.setString(3, endpointHash(subscription.endpoint))
        statement.setString(4, subscription.p256dhKey)
        statement.setString(5, subscription.authKey)
        statement.setInt(6, subscription.createdCycle)
        statement.setInt(7, subscription.lastPushedNotificationId)
        statement.setInt(8, subscription.failureCount)
        subscription.userAgent match {
          case Some(value) => statement.setString(9, value.take(512))
          case None => statement.setNull(9, Types.VARCHAR)
        }
  ...
  def loadByEndpoint(endpoint: String): Option[PushSubscription] = {
    Using.resource(Meta.getConnection()) { connection =>
      ensureTable(connection)
      Using.resource(connection.prepareStatement(s"SELECT * FROM $PUSH_SUBSCRIPTION_TABLE WHERE endpoint_hash = ? AND endpoint = ?")) { statement =>
        statement.setString(1, endpointHash(endpoint))
  ...
  def deleteByEndpoint(airlineId: Int, endpoint: String): Int = {
    Using.resource(Meta.getConnection()) { connection =>
      ensureTable(connection)
      Using.resource(connection.prepareStatement(s"DELETE FROM $PUSH_SUBSCRIPTION_TABLE WHERE airline = ? AND endpoint_hash = ? AND endpoint = ?")) { statement =>
        statement.setInt(1, airlineId)
        statement.setString(2, endpointHash(endpoint))
